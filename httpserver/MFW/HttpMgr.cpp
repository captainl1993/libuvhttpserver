...
void HttpMgr::OnRead(uv_stream_t * session, std::string data, NNTCPNode& netNode)
{
...
void HttpMgr::OnConnected(uv_stream_t * session, NNTCPNode& netNode)
{
...
void HttpMgr::OnDisConnected(uv_stream_t * session, NNTCPNode& netNode)
{
...
void HttpMgr::SendDataClient(uv_stream_t * session, std::string data, NNTCPNode& netNode, std::map<std::string, std::string> params, std::string Url)
{
	if (netNode.nNNodeInfo.isClient)
	{
		HttpRequest httpRequest;
...
void HttpMgr::SendData(uv_stream_t * session, std::string data, std::map<std::string, std::string> params, NNTCPNode& netNode)
{
	if (!netNode.nNNodeInfo.isClient)
	{
		HttpResPonse httpResPonse;
